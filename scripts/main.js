...
(function () {
    ...
    function applyTheme(theme) {
        if (theme === 'dark') {
            ...
            themeToggles.forEach(button => button.setAttribute('aria-pressed', 'true'));
            themeToggles.forEach(button => button.setAttribute('aria-label', 'Alternar para modo claro'));
            toggleTexts.forEach(text => text.textContent = 'Modo Claro'); // Exemplo para mobile
        } else {
            docElement.classList.remove('dark');
            sunIcons.forEach(icon => icon.classList.add('hidden'));
            moonIcons.forEach(icon => icon.classList.remove('hidden'));
            themeToggles.forEach(button => button.setAttribute('aria-pressed', 'false'));
            themeToggles.forEach(button => button.setAttribute('aria-label', 'Alternar para modo escuro'));
            toggleTexts.forEach(text => text.textContent = 'Modo Escuro'); // Exemplo para mobile
        }
    }
    function toggleTheme() {
        const currentTheme = docElement.classList.contains('dark') ? 'light' : 'dark';
